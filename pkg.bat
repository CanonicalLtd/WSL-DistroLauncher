...
if not exist "%_OWNER%".pfx (
    ...
    %_KITS%\makecert -r -h 0 -n "CN=23596F84-C3EA-4CD8-A7DF-550DCE37BCD0" -eku "1.3.6.1.5.5.7.3.3,1.3.6.1.4.1.311.10.3.13" -pe -sv %_OWNER%.pvk %_OWNER%.cer
...
%_KITS%\makeappx.exe pack /m MyDistro.appxmanifest /f filemap.txt /p Ubuntu.appx
%_KITS%\signtool sign /v /f "%_OWNER%".pfx /fd SHA256 Ubuntu.appx

%_KITS%\makeappx.exe pack /m MyDistro.appxmanifest /f filemap-empty.txt /p UbuntuEmpty.appx
%_KITS%\signtool sign /v /f "%_OWNER%".pfx /fd SHA256 UbuntuEmpty.appx
